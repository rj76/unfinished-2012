...
var TWEEN = TWEEN || function () {
    var a, e, c = 60, b = false, h = [], i;
    return{setFPS:function (f) {
        c = f || 60
    }, start:function (f) {
        ...
        a = 0;
        i = h.length;
        for (f = f || Date.now(); a < i;)if (h[a].update(f))a++;
        else {
            h.splice(a, 1);
            i--
        }
        i == 0 && b == true && this.stop()
    }}
}();
TWEEN.Tween = function (a) {
    var e = {}, c = {}, b = {}, h = 1E3, i = 0, f = null, n = TWEEN.Easing.Linear.EaseNone, k = null, l = null, m = null;
    this.to = function (d, g) {
        if (g !== null)h = g;
        for (var j in d)if (a[j] !== null)b[j] = d[j];
        return this
    };
    this.start = function (d) {
        TWEEN.add(this);
        f = d ? d + i : Date.now() + i;
        for (var g in b)if (a[g] !== null) {
            e[g] = a[g];
            c[g] = b[g] - a[g]
        }
        return this
...
TWEEN.Easing.Elastic.EaseInOut = function (a) {
    ...
    if ((a *= 2) < 1)return-0.5 * c * Math.pow(2, 10 * (a -= 1)) * Math.sin((a - e) * 2 * Math.PI / b);
